# AI Usage Policy

**AI Usage & Disclaimer Policy**

**Effective Date:** 31 July 2026

## 1. Purpose

This AI Usage & Disclaimer Policy explains how Discover Adelaide Korea ("DAK") uses Artificial Intelligence ("AI") features and outlines the limitations of AI-generated content.

Our goal is to provide helpful, timely, and accessible information while encouraging users to verify important decisions using official sources.

## 2. AI Features

DAK may provide AI-powered features including, but not limited to:

-   AI Search

-   AI Local Assistant

-   AI Business Recommendations

-   AI Content Summaries

-   AI Guide Summaries

-   AI Translation Assistance

-   AI Personalised Recommendations

-   Australia Updates AI Titles and Summaries

-   AI-Generated Illustrations for Social Cards

The features available may change over time.

## 3. How AI Responses Are Generated

AI-generated responses may be based on:

-   Publicly available information

-   User queries

-   DAK content

-   Business listing information

-   Community content

-   Official sources where available

Some responses may be generated using third-party AI service providers. DAK currently uses Anthropic for text and OpenAI for image generation.

User prompts submitted to AI features may be processed by third-party AI service providers solely for the purpose of generating a response.

For information about how personal information is collected, used, and protected, please refer to our Privacy Policy.

## 4. Accuracy

Although DAK aims to provide reliable information:

-   AI responses may contain mistakes.

-   AI responses may be incomplete.

-   AI responses may become outdated.

-   AI may misunderstand user questions.

-   AI may occasionally generate information that appears accurate but is incorrect or misleading.

Users should independently verify important information.

## 5. Not Professional Advice

AI-generated content is provided for general informational purposes only.

It is **not** a substitute for professional advice.

This includes, but is not limited to:

-   Legal advice

-   Immigration advice

-   Medical advice

-   Financial advice

-   Tax advice

-   Mental health advice

-   Emergency services advice

Where appropriate, users should consult qualified professionals or the relevant government authority.

AI-generated content should not be relied upon as the sole basis for making important personal, legal, medical, financial, immigration, taxation, employment, or safety-related decisions.

## 6. Australia Updates

Australia Updates may include AI-assisted titles and summaries of publicly available information.

AI generates both the title and summary by rewriting factual information from the original source. DAK does not use AI to produce direct translations or reproduce the original article.

The purpose is to help users quickly understand important information while encouraging them to refer to the original source where appropriate.

Where reasonably possible, DAK will provide links or references to the original source.

Before publication, AI-generated titles and summaries may be reviewed and edited by a DAK administrator to improve factual accuracy, clarity, consistency and compliance with DAK editorial standards.

AI may also assist in determining whether publicly available content is relevant to DAK users before it is submitted for editorial review.

Final publication decisions remain under the control of DAK administrators.

Where DAK indicates that content is AI-generated, this refers to both the AI-generated title and AI-generated summary produced during the same generation process.

## 7. AI Search Results

AI Search may combine information from multiple sources.

Search results should be treated as informational assistance rather than verified facts.

Official government websites and original sources remain the authoritative reference.

## 8. Business Information

AI may summarise information about businesses listed on DAK.

DAK does not guarantee:

-   Current opening hours

-   Pricing

-   Availability

-   Promotions

-   Product availability

Users should confirm important details directly with the business.

Business information may change without notice and may not always reflect the most recent updates.

## 9. Community Content

AI may summarise or recommend community posts.

Community content reflects the opinions of individual users and does not necessarily represent the views of DAK.

## 10. User Responsibilities

When using AI features, users agree:

-   Not to rely solely on AI for important decisions.

-   To verify important information independently.

-   To use AI responsibly.

-   Not to use AI to generate illegal or harmful content through DAK.

## 11. Service Availability

AI features may occasionally be:

-   Modified

-   Limited

-   Temporarily unavailable

-   Permanently discontinued

DAK does not guarantee uninterrupted AI service availability.

DAK may suspend or restrict AI features without prior notice where necessary for maintenance, security, legal compliance, or service improvements.

## 12. Limitation of Liability

To the maximum extent permitted by applicable law, DAK is not responsible for losses arising from reliance on AI-generated information.

To the extent permitted by applicable law, DAK does not warrant that AI-generated content will always be accurate, complete, reliable, or suitable for any particular purpose.

Nothing in this Policy excludes rights that cannot be excluded under Australian consumer protection laws.


## 13. Improvements

AI systems continue to evolve.

DAK may improve AI-generated titles, summaries, relevance assessment and other AI-assisted features through feedback, testing and ongoing model improvements.

AI features may change as new technologies, regulations, and best practices evolve.

## 14. Changes to this Policy

We may update this Policy from time to time.

Material changes will be published on the DAK website.

Continued use of AI features after changes become effective constitutes acceptance of the updated Policy.

## 15. AI-Generated Images

DAK produces shareable image cards for some Australia Updates. The illustration or photograph on a card is generated by an AI image service. Everything else on the card — the Korean text, the figures, the layout and the branding — is produced by DAK from content an administrator has already reviewed.

**No image depicts a real event.** DAK does not use AI to recreate what happened, and does not present a generated image as a record of it.

Two visual treatments are used, chosen according to the subject:

-   A hand-painted illustration style for most updates. It is plainly a drawing, which is the point.

-   A photographic treatment for updates where a single verified figure is the main fact, and for reports of death, injury or disaster. This treatment is restricted to isolated objects. It never depicts people, faces, buildings, streets, interiors, identifiable places, crowds, emergency scenes, damage or injury, so that a generated image cannot be mistaken for documentary evidence of the event it accompanies.

Every card carries the words **AI 생성 이미지** ("AI-generated image") beside the artwork. This is printed into the image itself rather than added to a caption, so that it remains present when the card is screenshotted, forwarded or shared to another platform.

AI also selects how a card is arranged, including whether a figure is shown prominently. Where a card reports harm to people, the arrangement is deliberately restrained: a verified figure is stated plainly rather than enlarged for effect.

Cards are reviewed by a DAK administrator before publication. Where an image is unsuitable, it is regenerated or the card is not published.

## 16. Contact

Questions regarding this AI Usage & Disclaimer Policy may be submitted to admin@discoveradelaidekorea.au.

*Last Updated: 11 August 2026*
