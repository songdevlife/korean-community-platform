...
function AdminPage() {
  ...
  }, [businessPage, draftPage, publishedPage, draftGuidePage, publishedGuidePage]);
  ...
  // Admin is reached from My Page, so it gets a back control like the other
  // second-level screens. Repeated across states because the error branch in
  // particular must not trap someone who lacks permission.
  const backButton = (
    <button
      onClick={() => navigate(-1)}
      aria-label="Back"
      ...
      <Layout>
        <PageShell>
          ...
          <div className="rounded-2xl border border-border-dark bg-night p-8 text-center">
            ...
            <p className="text-snow text-sm">{error}</p>
    ...
    <Layout>
      ...
      <PageMeta title="Admin" noIndex />
        ...
        <section className="mb-8">
          <h2 className="text-lg font-semibold text-snow mb-3">
            Draft updates{' '}
            <span className="text-muted font-normal">({draftTotal})</span>
          </h2>
            ...
            <>
              <div className="grid gap-2.5">
                {updates.map((update) => {
                  ...
                  // them, so the controls below let an admin supply each before
                  // the publish button unlocks. The summary is the one that
                  // matters most: without it, publishing would put the
                  // publisher's own article text on the site.
                  const missing = [
                      ...
                      {isEditing ? (
                        <div className="flex flex-col gap-3">
                          ...
                          {/* Source text, read-only. This is the publisher's own
                              article and is never published — it is here to
                              write the summary from. Kept in a scroll box so a
                              long article does not bury the field below it. */}
                          ...
                          <div className="flex gap-2">
                            {/* An empty summary saves fine — a half-written
                                draft has to be resumable. Publication is where
                                it is refused. */}
                            <button
        ...
        <section>
          <h2 className="text-lg font-semibold text-snow mb-3">
            Published updates{' '}
            <span className="text-muted font-normal">({publishedTotal})</span>
          </h2>
        ...
        <section className="mt-8">
          <div className="flex items-center justify-between gap-4 mb-3">
            ...
            <Link to="/admin/guides/new" className={`${secondaryBtn} shrink-0`}>
